...
#head(results_mat_seg)
#head(results_mat_nonseg)
...
#x = data.frame(data[,c('KNearest_Mean')])
#colnames(x) = 'KNearest_Mean'
#y = data[,c('Outcome')]

#Naive Bayes
#model = train(x,y,'nb',trControl=trainControl(method='cv',number=10)) #this doesn't run; error installing package
model <- naiveBayes(Outcome ~ KNearest_Mean, data = data)
#model #not clear on what this is giving me . . .
#pred = predict(model, newdata = data$KNearest_Mean) #predictions for full data set. DOESN'T WORK; always 0
pred = predict(model, newdata = data) #predictions for full data set
#s(pred) #predicts 0 every time??? something terribly wrong

#sum(pred == data$Outcome) #correct 1919 times . . . Not bad!

#try this again on test, training data
model <- naiveBayes(Outcome ~ KNearest_Mean, data = data[1:(nrow(data)-101),])
pred = predict(model, newdata = data[(nrow(data)-100):nrow(data),])
